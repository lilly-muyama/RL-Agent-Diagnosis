SEED = 42
MAX_STEPS = 10
# ACTION_SPACE = ['No anemia', 'Vitamin B12/Folate deficiency anemia', 'Unspecified anemia', 'Anemia of chronic disease', 
# 'Iron deficiency anemia', 'Hemolytic anemia', 'Aplastic anemia', 'hemoglobin', 'ferritin', 'ret_count', 'segmented_neutrophils',
# 'serum_iron', 'tibc', 'rbc', 'mcv', 'age', 'gender', 'indirect_bilirubin', 'transferrin', 'creatinine', 'cholestrol', 'copper', 
# 'ethanol', 'folate', 'glucose']

ACTION_SPACE = ['No anemia', 'Vitamin B12/Folate deficiency anemia', 'Unspecified anemia', 'Anemia of chronic disease', 
'Iron deficiency anemia', 'Hemolytic anemia', 'Aplastic anemia', 'Inconclusive diagnosis', 'hemoglobin', 'ferritin', 'ret_count', 
'segmented_neutrophils', 'tibc', 'mcv', 'serum_iron', 'rbc', 'age', 'gender', 'indirect_bilirubin', 'transferrin', 'creatinine', 
'cholestrol', 'copper', 'ethanol', 'folate', 'glucose']
DT_ACTION_SPACE = ['hemoglobin', 'ferritin', 'ret_count', 'segmented_neutrophils', 'tibc', 'mcv', 'gender']
ACTION_NUM = len(ACTION_SPACE)
CLASS_DICT = {'No anemia': 0, 'Vitamin B12/Folate deficiency anemia': 1, 'Unspecified anemia': 2, 'Anemia of chronic disease': 3, 
'Iron deficiency anemia': 4, 'Hemolytic anemia': 5, 'Aplastic anemia': 6, 'Inconclusive diagnosis': 7}
CLASS_NUM = len(CLASS_DICT)
FEATURE_NUM = len(ACTION_SPACE) - CLASS_NUM