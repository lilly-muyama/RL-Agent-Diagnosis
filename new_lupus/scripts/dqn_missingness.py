...
def run_dqn_model(model_type, seed, steps, missingness_level):
    # dir_name = f'seed_{seed}_{steps}'
    dir_name = f'seed_{seed}_{steps}'
    parent_dir = f'../models/logs/{model_type}/missingness/{missingness_level}/biopsy_3'
...
if __name__ == '__main__':
    ...
    parser.add_argument('-m', '--missingness', help='Level of missingness in the dataset', type=float, default=0.1) #added
    ...
    train_df = pd.read_csv(f'../new_data/with_inconclusive_diagnosis/train_set_missingness_{args.missingness}.csv')
    ...
    for name in model_names:
        proc = Process(target=run_dqn_model, args=(name, constants.SEED, args.steps, args.missingness))
        procs.append(proc)
        proc.start()
